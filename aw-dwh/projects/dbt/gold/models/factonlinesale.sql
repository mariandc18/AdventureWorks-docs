...
select {{ dbt_utils.generate_surrogate_key(
    ...
    sod.salesorderdetailid,
    sod.specialofferid,
    soh.onlineorderflag as isonline,
    ...
    case
        when dp.weightunitmeasurecode = 'LB' then greatest(sm.shiprate * dp.weight, sm.shipbase)
        when dp.weightunitmeasurecode = 'G' then greatest(
            sm.shiprate * dp.weight * 0.00220462,
            sm.shipbase
        )
        else sm.shipbase
    end as freight
