...
routing_detail as (
    select 
        ...
        fwo.orderqty,
        fwo.is_late,
        case when fwo.scrappedqty > 0 then 1 else 0 end as has_defect
...
select 
    ...
    count(distinct case when has_defect = 1 then workorderid end) as orders_with_defects,
    count(distinct case when is_late = 1 then workorderid end) as late_orders,
    round(sum(plannedcost), 2) as total_planned_cost,
    round(sum(actualcost), 2) as total_actual_cost
    
from routing_detail
group by operation_date, operationsequence, location_name, product_name